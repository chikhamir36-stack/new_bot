...
async def level_roles(ctx):
    ...
    for role_name, level in sorted(level_rewards.items(), key=lambda x: x[1]):
        ...
        if role:
            desc += f"{role.mention} → Level **{level}**\n"
        else:
            desc += f"**{role_name}** → Level **{level}** ⚠️\n"
...
async def level(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="📊 Progress", value=f"**{int((xp / needed) * 100)}%**", inline=True)
    embed.set_footer(text="Keep chatting and staying in voice!")
    await ctx.send(embed=embed)

# ==========================
# 🎯 أمر !leaderboard_level
# ==========================
...
async def lvl_up(ctx):
    ...
    embed.add_field(
        name="🎖️ Ranks",
        value=f"**{sum(1 for r, l in level_rewards.items() if user['level'] >= l)}** / **{len(level_rewards)}** unlocked",
        inline=True
    )
    embed.add_field(
        name="📌 Commands",
...
# ==========================
# 👋 نظام تتبع الدعوات (Invite Tracker)
# ==========================
...
async def load_invite_cache():
    """تحميل الدعوات في الكاش عند التشغيل"""
    global invite_cache
    for guild in bot.guilds:
        try:
            invites = await guild.invites()
    ...
    try:
        ...
        for invite in new_invites:
            ...
            if old_invite:
                ...
                # إذا كانت دعوة جديدة (ما كانتش في الكاش)
                if invite.uses > 0:
                    inviter = invite.inviter
                    break
        ...
        # ===== نرسل رسالة للداعي في الـ DM (اختياري) =====
        if inviter and not inviter.bot:
            try:
                dm_embed = discord.Embed(
                    title="🎉 New Invite!",
                    description=f"{member.mention} joined the server using your invite!",
                    color=discord.Color.green()
                )
        ...
        print(f"❌ Error in the invitation system : {e}")
# ==========================
# RUN BOT
# ==========================
